...
const UserProfile = () => {
  ...
  return (
    <div className="flex justify-center w-max mx-auto items-center min-h-[50vh] lg:min-h-[calc(100vh-95px)]">
      <Helmet>
        ...
        <div className="space-y-4 text-center divide-y divide-gray-700">
          <div className="my-2 px-3 flex flex-col items-center">
            ...
            <div className="flex flex-col justify-center items-start">
              <div>
                {
                  user?.email ? <h2 title={user?.email} className="text-[16px] text-[#EEE0FFB3] mt-3 flex items-center gap-2">
                  <MdOutlineMailOutline></MdOutlineMailOutline>{" "}
                  {user?.email?.length > 22
                    ? user?.email?.slice(0, 21) + "..."
                    : user?.email}
                </h2> :
                <h2 className="text-[16px] text-[#EEE0FFB3] mt-3 flex items-center gap-2"><MdOutlineMailOutline></MdOutlineMailOutline> Now now</h2>
                }
              </div>
